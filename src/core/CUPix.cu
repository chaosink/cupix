...
__global__
void Font(int ch, int x0, int y0, bool ssaa, unsigned char *frame_buf) {
	int x = threadIdx.x + blockIdx.x * blockDim.x;
	int y = threadIdx.y + blockIdx.y * blockDim.y;
...
void CUPix::UnmapResources() {
	if(aa_ != NOAA) cu::DownSample<<<dim3((window_w_-1)/32+1, (window_h_-1)/32+1), dim3(32, 32)>>>(frame_buf_, pbo_buf_);
...
void CUPix::DrawFPS(int fps) {
	bool aa = aa_ != NOAA;
	cu::Font<<<1, dim3(16, 16)>>>('F',  0, 0, aa, frame_buf_);
	cu::Font<<<1, dim3(16, 16)>>>('P', 16, 0, aa, frame_buf_);
	cu::Font<<<1, dim3(16, 16)>>>('S', 32 - 3, 0, aa, frame_buf_);
	cu::Font<<<1, dim3(16, 16)>>>(fps % 1000 / 100 + 48, 48 + 5, 0, aa, frame_buf_);
	cu::Font<<<1, dim3(16, 16)>>>(fps % 100 / 10   + 48, 64 + 5, 0, aa, frame_buf_);
	cu::Font<<<1, dim3(16, 16)>>>(fps % 10         + 48, 80 + 5, 0, aa, frame_buf_);
}
